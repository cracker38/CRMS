...
const SiteSupervisorDashboard = ({ activeTab: propActiveTab, onTabChange, onRefreshNotifications }) => {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      // Header (standardized to match Purchase Order report)
      ...
      const generatedByName =
        ...
        user?.email ||
        'User';
      const generatedBy = `Generated by: ${generatedByName}`;
      const g1w = doc.getTextWidth(generatedOn);
      doc.text(generatedOn, (pageWidth - g1w) / 2, yPos);
      yPos += 6;
      const g2w = doc.getTextWidth(generatedBy);
      doc.text(generatedBy, (pageWidth - g2w) / 2, yPos);
      yPos += 10;
                  ...
                  <div className="card">
                    ...
                    <div className="card-body">
                      {Array.from(
                        new Set(
                          (sites || [])
                            .map((s) => (s.project_name || '').toString().trim())
                            .filter(Boolean)
                        )
                      ).length > 0 ? (
                        <ul className="mb-0">
                        ...
                        <p className="text-muted mb-0">No projects assigned yet.</p>
                      )}
                    </div>
                  </div>
                </div>
